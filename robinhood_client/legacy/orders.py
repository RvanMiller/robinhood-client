...
from .helper import (
    error_argument_not_key_in_dictionary,
    filter_data,
    get_output,
    id_for_option,
    login_required,
...
def order_buy_fractional_by_price(
    ...
    if amountInDollars < 1:
        print(
            "ERROR: Fractional share price should meet minimum 1.00.", file=get_output()
        )
        return None
    ...
    fractional_shares = (
        0 if (price == 0.00) else round_price(amountInDollars / float(price))
    )
...
def order_trailing_stop(
    ...
    if trailType == "amount":
        payload["trailing_peg"] = {
            "type": "price",
            "price": {"amount": trailAmount, "currency_code": "USD"},
        }
    else:
...
def order(
    ...
    if limitPrice and stopPrice:
        ...
        price = round_price(
            next(iter(get_latest_price(symbol, priceType, extendedHours)), 0.00)
        )
    ...
    payload = {
        ...
        "symbol": symbol,
        "price": price,
        "ask_price": round_price(
            next(iter(get_latest_price(symbol, "ask_price", extendedHours)), 0.00)
        ),
        "bid_ask_timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f"),
        "bid_price": round_price(
            next(iter(get_latest_price(symbol, "bid_price", extendedHours)), 0.00)
        ),
        "quantity": quantity,
        "ref_id": str(uuid4()),
        "type": orderType,
    ...
    if market_hours == "regular_hours":
        ...
        payload["quantity"] = int(
            payload["quantity"]
        )  # round to integer instead of fractional
...
@login_required
def order_option_credit_spread(
    price,
    symbol,
    quantity,
    spread,
    ...
    return order_option_spread(
        "credit", price, symbol, quantity, spread, timeInForce, account_number, jsonify
    )


@login_required
def order_option_debit_spread(
    price,
    symbol,
    quantity,
    spread,
    ...
    """
    return order_option_spread(
        "debit", price, symbol, quantity, spread, timeInForce, account_number, jsonify
    )
...
def order_buy_crypto_by_price(symbol, amountInDollars, timeInForce="gtc", jsonify=True):
    ...
    """
    return order_crypto(
        symbol, "buy", amountInDollars, "price", None, timeInForce, jsonify
    )
...
@login_required
def order_buy_crypto_limit(
    symbol, quantity, limitPrice, timeInForce="gtc", jsonify=True
):
    ...
    """
    return order_crypto(
        symbol, "buy", quantity, "quantity", limitPrice, timeInForce, jsonify
    )


@login_required
def order_buy_crypto_limit_by_price(
    symbol, amountInDollars, limitPrice, timeInForce="gtc", jsonify=True
):
    ...
    """
    return order_crypto(
        symbol, "buy", amountInDollars, "price", limitPrice, timeInForce, jsonify
    )


@login_required
def order_sell_crypto_by_price(
    symbol, amountInDollars, timeInForce="gtc", jsonify=True
):
    """Submits a market order for a crypto by specifying the amount in dollars that you want to trade.
    Good for share fractions up to 8 decimal places.
    ...
    """
    return order_crypto(
        symbol, "sell", amountInDollars, "price", None, timeInForce, jsonify
    )
...
def order_sell_crypto_by_quantity(symbol, quantity, timeInForce="gtc", jsonify=True):
    ...
    """
    return order_crypto(
        symbol, "sell", quantity, "quantity", None, timeInForce, jsonify
    )


@login_required
def order_sell_crypto_limit(
    symbol, quantity, limitPrice, timeInForce="gtc", jsonify=True
):
    ...
    """
    return order_crypto(
        symbol, "sell", quantity, "quantity", limitPrice, timeInForce, jsonify
    )


@login_required
def order_sell_crypto_limit_by_price(
    symbol, amountInDollars, limitPrice, timeInForce="gtc", jsonify=True
):
    ...
    """
    return order_crypto(
        symbol, "sell", amountInDollars, "price", limitPrice, timeInForce, jsonify
    )
