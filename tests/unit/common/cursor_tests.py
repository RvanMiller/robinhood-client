...
from robinhood_client.common.cursor import (
    Cursor,
    ApiCursor,
    PaginatedResult,
    CursorResponse,
...
class TestCursor:
    ...
    def test_cursor_current_page_fetches_first_page(self):
        ...
        mock_response = CursorResponse(
            results=[{"id": "1"}], next="next_url", previous=None
        )
    ...
    def test_cursor_next_page(self):
        ...
        first_page = CursorResponse(
            results=[{"id": "1"}], next="next_url", previous=None
        )
        second_page = CursorResponse(
            results=[{"id": "2"}], next=None, previous="prev_url"
        )
    ...
    def test_cursor_iteration(self):
        ...
        page1 = CursorResponse(
            results=["item1", "item2"], next="next_url", previous=None
        )
        page2 = CursorResponse(
            results=["item3", "item4"], next=None, previous="prev_url"
        )
    ...
    def test_cursor_first(self):
        ...
        mock_response = CursorResponse(
            results=["item1", "item2"], next=None, previous=None
        )
        fetch_func = Mock(return_value=mock_response)
...
class TestApiCursor:
    ...
    def test_api_cursor_fetch_with_endpoint(self):
        ...
        mock_client = Mock()
        mock_client.request_get.return_value = {
            "results": [],
            "next": None,
            "previous": None,
        }
        ...
        mock_client.request_get.assert_called_once_with(
    ...
    def test_api_cursor_fetch_with_cursor_url(self):
        ...
        mock_response1 = {
            "results": ["item1"],
            "next": "http://api.example.com/test/?cursor=abc",
            "previous": None,
        }
        mock_response2 = {
            "results": ["item2"],
            "next": None,
            "previous": "http://api.example.com/test/?cursor=def",
        }
        ...
        mock_client.request_get.assert_any_call(
            "http://api.example.com/test/?cursor=abc"
        )
