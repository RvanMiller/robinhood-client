...
def test_get_options_orders(authenticated_options_client: OptionsDataClient, account_number: str):
    ...
    for order in result.results:
        ...
        assert first_leg.position_effect in ['open', 'close'], f"Invalid position_effect: {first_leg.position_effect}"
        
        print(f"✓ Order {order.id}: {first_leg.side} {first_leg.position_effect} {first_leg.option_type} - {order.state}")


def test_get_specific_options_order(authenticated_options_client: OptionsDataClient, account_number: str):
    ...
    required_fields = [
        "id", "ref_id", "account_number", "chain_id", "chain_symbol"
    ]
    ...
    print(f"✓ Single options order validation passed - ID: {specific_order.id}, "
...
def test_get_options_orders_cursor_iteration(authenticated_options_client: OptionsDataClient, account_number: str):
    ...
    print(f"✓ Cursor iteration: {len(all_orders)} options orders across {page_count} pages")


def test_get_options_orders_with_date_string(authenticated_options_client: OptionsDataClient, account_number: str):
    ...
    print(f"Date string test: Found {len(result.results)} options orders since {start_date_str}")
